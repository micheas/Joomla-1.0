...
class HTML_contact
{
					...
					<td width="60%" valign="top" class="contentdescription<?php echo $params->get('pageclass_sfx'); ?>"
						...
						if ($currentcat->img)
						{
							...
							<img src="<?php echo $currentcat->img; ?>" align="<?php echo $currentcat->align; ?>"
							     hspace="6" alt="<?php echo _WEBLINKS_TITLE; ?>"/>
						<?php
						}
	...
	function viewcontact(&$contact, &$params, $count, &$list, &$menu_params)
	{
		...
		<table width="100%" cellpadding="0" cellspacing="0" border="0"
		       class="contentpane<?php echo $menu_params->get('pageclass_sfx'); ?>">
			<?php
			// displays Page Title
		...
		if (($count > 1) && !$params->get('popup') && $params->get('drop_down'))
		{
			...
			<tr>
				<td align="center">
					...
					<form action="<?php echo sefRelToAbs('index.php?option=com_contact&amp;Itemid=' . $Itemid); ?>"
					      method="post" name="selectForm" target="_top" id="selectForm">
	...
	function _writeImage(&$contact, &$params)
	{
		...
		if ($contact->image && $params->get('image'))
		{
			...
			<div style="float: right;">
				<img src="<?php echo $mosConfig_live_site; ?>/images/stories/<?php echo $contact->image; ?>"
				     align="middle" alt="<?php echo _CONTACT_TITLE; ?>"/>
			</div>
		<?php
		}
	}
				...
				<td colspan="2">
					...
					<form action="<?php echo sefRelToAbs('index.php?option=com_contact&amp;Itemid=' . $Itemid); ?>"
						...
						<div class="contact_email<?php echo $menu_params->get('pageclass_sfx'); ?>">
							...
							<br/>
							<br/>
							<input type="button" name="send" value="<?php echo(_SEND_BUTTON); ?>" class="button"
							       onclick="validate()"/>
						</div>
