		...
		<form action="index2.php" method="post" name="adminForm">
			...
			<table class="adminform">
				...
				<tr>
					...
					<td>
						<input type="text" name="vars[auto_purge]" size="5" value="<?php echo $vars['auto_purge']; ?>"
						       class="inputbox"/> days old
					</td>
				</tr>
			</table>
	...
	function viewMessage(&$row, $option)
	{
		...
		<form action="index2.php" method="post" name="adminForm">
			...
			<input type="hidden" name="subject"
			       value="<?php echo (substr($row->subject, 0, 4) != 'Re: ' ? 'Re: ' : '') . htmlspecialchars($row->subject, ENT_QUOTES); ?>"/>
			<input type="hidden" name="hidemainmenu" value="0"/>
			<input type="hidden" name="<?php echo josSpoofValue(); ?>" value="1"/>
		</form>
	<?php
