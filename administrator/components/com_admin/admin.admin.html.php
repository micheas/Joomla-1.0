	...
	*/
	...
	function fullCheck() {
		...
		if ( $type ) {
			...
			$url				= 'http://www.joomla.org/cache/versioncheck'. $versioninfo .'.xml';
		}
						...
						if ($type) {
							if ($totalItems > 0) {				
								...
								if (!isset($data['major']) || !isset($data['minor']) ) {
									...
									$data['width']	= '520';
									if ($outofdate == 1) {
										$data['text'] 	= 'OUT OF DATE';
										$data['colour'] = 'red';
										$data['image']	= '<img src="../images/cancel_f2.png"  style="vertical-align: middle;" />';
									} else if ($outofdate == 0) {
										$data['text'] 	= 'UP-TO-DATE';
										$data['colour'] = 'green';
										$data['image']	= '<img src="../images/apply_f2.png"  style="vertical-align: middle;" />';				
									}
									
									HTML_admin_misc::versionStatusHTML( $data );									
														
									HTML_admin_misc::versionInfoHTML( $data, $_VERSION, $outofdate );									
								}	
							} else {
								HTML_admin_misc::unableConnectHTML();
							}
						} else {
							$outofdate 	= 0;
	 ...
	 * Added 1.0.11
	 */
	function versionStatusHTML( $data ) {
		?>
	 ...
	 * Added 1.0.11
	 */
	function versionInfoHTML( $data, $_VERSION, $outofdate ) {
		if (!isset($data['major'])) {
			$data['major'] = '1.0';
		}			
		if (!isset($data['minor'])) {
